    ...
    def net(self):
        ...
        for i in range(0, len(self.list1)):
            ...
            try:
                ...
                try:
                    df = pro.moneyflow(ts_code='{}'.format(self.list1[i]), start_date=self.day_str, end_date = self.day_str)
    ...
    def market(self):
        ...
        for i in range(0, len(self.list1)):
            ...
            try:
                ...
                try:
                    df = pro.daily_basic(ts_code='{}'.format(self.list1[i]), start_date=self.day_str, end_date=self.day_str)
                    for k in df.circ_mv:
                        list2.append(k)
                    o = 0
                    for j in df.trade_date:
                ...
                try:
                    ...
                    for row in ma_update.items():
                        if not isnan(row[1]):
                            ...
                            value = row[1]
                            self.engine.execute(
                                "update {} set ma{} = {} where trade_date = {}".format(stock, num2, value, '20211012'))
                except Exception as e:
                    print(e)
