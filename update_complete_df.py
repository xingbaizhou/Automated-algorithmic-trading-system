...
class Handle(object):
    ...
    def q(self):
        ...
        try:
            for i in self.list1:
                print(i)
                try:
                    data = pro.daily(ts_code=i, start_date='20211013', end_date=self.day_str)
                    print(data)
                    if not data.empty:
    ...
    def net(self):
        ...
        for i in range(0, len(self.list1)):
            ...
            try:
                ...
                try:
                    df = pro.moneyflow(ts_code='{}'.format(self.list1[i]), start_date='20211013', end_date = self.day_str)
    ...
    def market(self):
        ...
        for i in range(0, len(self.list1)):
            ...
            try:
                ...
                try:
                    df = pro.daily_basic(ts_code='{}'.format(self.list1[i]), start_date='20211013', end_date=self.day_str)
                    for k in df.circ_mv:
                        list2.append(k)
                    o = 0
                    for j in df.trade_date:
    ...
    def update_ma_daily(self, num2):
        """
        num2:多少日均线的数字,加字段,算均线,需要设置日期,日期从之前到现在必须从上往下排列
        """
        #stock  table 000001_sz
        for stock in self.stock_list:
            try:
                ...
                try:
                    #设置日期
                    stock_data = pd.read_sql("select * from (select * from {} order by trade_date desc limit {}) t1 order by t1.trade_date".format(stock,num2), index_col='trade_date',
                    ...
                    for row in ma_update.items():
                        if not isnan(row[1]):
                            ...
                            value = row[1]
                            self.engine.execute(
                                "update {} set ma{} = {} where trade_date = {}".format(stock, num2, value, trade_date))
                except Exception as e:
                    print(e)

    def update_ma_delta(self, num1, num2):
        """
        num1,num2差距,数字小的在前,必须两列都存在,5,60, (ma5-ma60 / ma60), ma12,ma50
        """
        for stock in self.stock_list:
            try:
    ...
    def inflows_over_market_equity_rank(self, write_csv=False, write_json=False):
        """
        return the inflow/market equity value of the last five days. 资金流入/市值
        :param ts_code:
        :return:
        """
    ...
    def l3_x_ratio(self, write_csv=False, write_json=False):
        ...
        # LEVEL3
        now = datetime.date.today()
        #用overall找出各行业前三,stock_industries,sw_list,industry_l3,X
        ...
        if df_test.empty:
            ...
            for rows in data.iterrows():
                ...
                df_exist = pd.read_sql("select * from life_cycle_stats where ts_code = '{}'".format(ts_code),
                                       con=self.conn)
                table_name = ts_code.replace('.', '_')
                print(table_name,file_date)
                chg_df = pd.read_sql("select pct_chg from {} where trade_date = {}".format(table_name, self.day_str),
                                     con=self.conn)
                print(chg_df)
                ########################################
                # 2
    ...
    def start_update(self):
        ...
        self.update_ma_delta(num1=5, num2=20)
        self.update_ma_delta(num1=20, num2=60)
        self.l3_x_ratio(write_csv=True, write_json=True)
        self.life_cycle(self.day_str)
        self.pull_sql_stats()
        self.daily_plot_json()

    def update_news(self):
        update_list = self.symbols
